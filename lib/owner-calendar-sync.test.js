...
function loadOwner({ captured = [], getOtaBlockedDates, listOwnerConnections }) {
  ...
  require.cache[PATHS.calendarView] = {
    ...
    exports: {
      ...
      normalizeView: realCalendarView.normalizeView,
...
describe('owner calendar_sync / calendar_view / calendar_feeds_status', () => {
  it('calendar_sync passes the same view params as calendar_view plus syncMode full_refresh', async () => {
    ...
    const cases = [
      { view: 'week', year: 2026, month: 9, focusDate: '2026-09-10' },
      { view: 'year', year: 2026, month: 9, focusDate: '2026-09-10' },
      { view: 'day', year: 2026, month: 9, focusDate: '2026-09-12' }
    ];
    for (const body of cases) {
      ...
      assert.deepEqual(captured[captured.length - 2], { ...body, syncMode: 'view' });
      assert.deepEqual(captured[captured.length - 1], { ...body, syncMode: 'full_refresh' });
      assert.equal(viewRes.body.sync.mode, 'view');
      assert.equal(syncRes.body.sync.mode, 'full_refresh');
      assert.equal(viewRes.body.view, body.view);
      assert.equal(syncRes.body.view, body.view);
      assert.equal(viewRes.body.sourceHealth, undefined);
      assert.equal(syncRes.body.sourceHealth, undefined);
    }
  });

  it('unknown calendar view falls back to month and is still passed to both actions', async () => {
    const captured = [];
    const handler = loadOwner({ captured });
    const res = mockRes();
    await handler({
      method: 'POST',
      headers: {},
      body: { action: 'calendar_view', view: 'agenda', year: 2026, month: 9, focusDate: '2026-09-10' }
    }, res);
