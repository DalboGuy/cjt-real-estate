...
const { listReservations, resolveReservationWindow } = require('./reservation-queries');
...
const FINANCIALS_PACING_DAYS = 90;
const FINANCIALS_PERIOD_PRESETS = ['year', 'ytd', 'last12', '2025', 'month'];
...
function extendFinancialsWindowForPacing(window, now = new Date()) {
  const today = todayKeyInTimeZone(now);
  const pacingTo = addDays(today, FINANCIALS_PACING_DAYS - 1);
  if (!window?.from) return { from: '', to: null };
  if (!window.to) return { from: window.from, to: null };
  return { from: window.from, to: laterDate(window.to, pacingTo) };
}

function resolveFinancialsQueryWindow({ from, to, now } = {}) {
  const when = now instanceof Date ? now : new Date();
  const requestedFrom = String(from || '').trim();
  const requestedTo = String(to || '').trim();
  if (requestedFrom || requestedTo) {
    return extendFinancialsWindowForPacing(
      resolveReservationWindow({ from: requestedFrom, to: requestedTo, now: when }),
      when
    );
  }
  const defaultWindow = resolveReservationWindow({ now: when });
  const periodFroms = FINANCIALS_PERIOD_PRESETS.map((preset) => dateRangeForPreset(preset, when).from);
  return {
    from: earliestDate([defaultWindow.from, ...periodFroms]),
    to: null
  };
}
...
async function loadImportedFinancials(sql, window, now = new Date()) {
  const resolved = window && window.from
    ? { from: window.from, to: window.to ?? null }
    : resolveFinancialsQueryWindow({ now });
  try {
    if (!(await tableExists(sql, 'booking_financials'))) {
      return { rows: [], available: false };
    }
    const rows = resolved.to
      ? await sql`
          ...
          WHERE checkout > ${resolved.from}::date
            AND checkin <= ${resolved.to}::date
...
async function loadOwnerFinancials(sql, now = new Date(), options = {}) {
  const { paymentSnapshotFromEvents } = require('./payments');
  const window = resolveFinancialsQueryWindow({
    from: options.from,
    to: options.to,
    now
  });
  const reservations = await listReservations(sql, {
    includeClosed: true,
    withQuote: true,
    from: window.from,
    to: window.to,
  ...
  `;
  const imported = await loadImportedFinancials(sql, window, now);
...
module.exports = {
  ...
  resolveFinancialsQueryWindow,
  FINANCIALS_PACING_DAYS,
  summarizeBookings,
  summarizePeriod,
  presentBooking,
  presentImportedStay,
