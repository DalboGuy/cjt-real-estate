...
function quoteStay(checkin,checkout,guests=1){
  ...
  return {
    currency:'USD',checkin,checkout,guests:Number(guests)||1,nights:nights.length,minimumStay,
    ...
    quoteVersion:'seasonal-v1',quotedAt:new Date().toISOString()
  };
}
...
function ownerAdjustedQuote(existing,lodgingSubtotal){
  ...
  return {
    ...
    taxes,total:round2(amount+cleaningFee+taxes),
    ownerAdjusted:true,quoteVersion:'owner-adjusted-v1',quotedAt:new Date().toISOString()
  };
}

module.exports={SEASONS,CLEANING_FEE,TAX_RATE,PRICING_THROUGH,eachDate,quoteStay,ownerAdjustedQuote};
