...
function parseDate(v){const m=String(v||'').match(/^(\d{4})(\d{2})(\d{2})(?:T\d{6}Z?)?$/);if(!m)return null;const d=new Date(Date.UTC(+m[1],+m[2]-1,+m[3]));return d.getUTCFullYear()===+m[1]&&d.getUTCMonth()===+m[2]-1&&d.getUTCDate()===+m[3]?d:null;}
...
async function readFeed(url,source){
  ...
  // An HTTP success can still contain an error page or a truncated export.
  // Never interpret an unreadable feed as an empty, available calendar.
  if(!/^BEGIN:VCALENDAR\r?$/m.test(text)||!/^END:VCALENDAR\r?$/m.test(text))throw new Error('invalid_calendar');
  const chunks=[...text.matchAll(/BEGIN:VEVENT([\s\S]*?)END:VEVENT/g)].map(x=>x[1]);
  if(chunks.length!==(text.match(/^BEGIN:VEVENT\r?$/gm)||[]).length||chunks.length!==(text.match(/^END:VEVENT\r?$/gm)||[]).length)throw new Error('incomplete_calendar');
  ...
  for(const ev of chunks){
    ...
    if(!startDate||!endDate||endDate<=startDate)throw new Error('invalid_calendar_event_dates');
