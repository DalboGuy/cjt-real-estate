...
function cleanText(v,max=120){return String(v||'').replace(/\\n/g,' ').replace(/\\,/g,',').replace(/\\;/g,';').replace(/\\\\/g,'\\').replace(/\s+/g,' ').trim().slice(0,max);}
function eventKind(summary){const s=String(summary||'').toLowerCase();return /reserved|booked|confirmed|closed/.test(s)?'reservation_like':'block';}
...
async function readFeed(url,source){
  const r=await fetch(url,{headers:{'user-agent':'CJT-Availability/2.3'}});
  ...
  for(const ev of chunks){
    ...
    const start=iso(startDate),end=iso(endDate),summary=cleanText(value(ev,'SUMMARY'))||'Unavailable',uid=value(ev,'UID');
    for(let d=new Date(startDate);d<endDate;d.setUTCDate(d.getUTCDate()+1))dates.add(iso(d));
    events.push({key:eventKey(source,start,end,uid,summary),source,start,end,summary,kind:eventKind(summary)});
  }
  return {dates,events};
}
