...
describe('preview-access shim',()=>{
  it('no longer uses an expired datetime; it follows ownerAuthOpen',()=>{
    const preview=require('./preview-access');
    assert.equal(preview.PREVIEW_PASSWORD_FREE_UNTIL,null);
    setFlag(undefined);
    assert.equal(preview.previewPasswordFreeActive(),true);
    assert.equal(preview.previewPasswordFreeSession().ownerAuthOpen,true);
    setFlag('1');
    assert.equal(preview.previewPasswordFreeActive(),false);
    assert.equal(preview.previewPasswordFreeSession(),null);
  });
});
