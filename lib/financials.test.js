...
function makeFinancialsSql({reservations=[],events=[],imported=[],bookingFinancialsExists=true}={}){
  const calls=[];
  function sql(strings,...values){
    const text=sqlText(strings);
    calls.push({text,values});
    if(/to_regclass/.test(text))return Promise.resolve([{relation:bookingFinancialsExists?'booking_financials':null}]);
    if(/FROM booking_financials/.test(text))return Promise.resolve(imported);
    if(/FROM booking_events/.test(text)&&/payment_/.test(text))return Promise.resolve(events);
    if(/FROM reservations/.test(text))return Promise.resolve(reservations);
    return Promise.resolve([]);
  }
  return {sql,calls};
}

function quotedStay(id,checkin,checkout,status='confirmed',total=1000){
  return {
    id,
    guest_name:id,
    ...
    checkout,
    status,
    quote:{lodgingSubtotal:total-240,cleaningFee:240,taxes:0,total,nights:3}
  };
}

describe('resolveFinancialsQueryWindow',()=>{
  const chicagoNow=new Date('2026-09-07T17:00:00.000Z');

  it('covers built-in period filters and leaves the future unbounded for pacing',()=>{
    const window=resolveFinancialsQueryWindow({now:chicagoNow});
    assert.equal(window.from,'2025-01-01');
    assert.equal(window.to,null);
    assert.equal(FINANCIALS_PACING_DAYS,90);
    assert.ok(window.from<='2025-01-01');
    assert.ok(window.from<=dateRangeForPreset('last12',chicagoNow).from);
    assert.ok(window.from<=dateRangeForPreset('year',chicagoNow).from);
    assert.ok(window.from<=dateRangeForPreset('ytd',chicagoNow).from);
  });

  it('extends an explicit period to through the 90-day pacing window',()=>{
    const window=resolveFinancialsQueryWindow({from:'2026-09-01',to:'2026-09-30',now:chicagoNow});
    assert.equal(window.from,'2026-09-01');
    assert.equal(window.to,addDays('2026-09-07',FINANCIALS_PACING_DAYS-1));
    assert.equal(window.to,'2026-12-05');
  });

  it('rejects inverted and invalid ranges with invalid_date_range',()=>{
    assert.throws(
      ()=>resolveFinancialsQueryWindow({from:'2026-12-31',to:'2026-01-01'}),
      (error)=>error.code==='invalid_date_range'&&error.status===400
    );
    assert.throws(
      ()=>resolveFinancialsQueryWindow({from:'2026-13-40'}),
      (error)=>error.code==='invalid_date_range'&&error.status===400
    );
  });
});

describe('financials stay window vs hard LIMIT',()=>{
  const chicagoNow=new Date('2026-09-07T17:00:00.000Z');

  it('keeps a stay beyond the old LIMIT 250 that overlaps the financials window',()=>{
    const past=Array.from({length:250},(_,index)=>({
      id:`old-${index}`,
      checkin:'2018-01-01',
      checkout:'2018-01-08',
      status:'confirmed'
    }));
    const future={id:'future-2028',checkin:'2028-12-20',checkout:'2028-12-27',status:'confirmed'};
    ...
    const window=resolveFinancialsQueryWindow({now:chicagoNow});
    const ranged=rows.filter((row)=>overlapsReservationWindow(row,window));
    assert.equal(ranged.some((row)=>row.id==='future-2028'),true);
    assert.equal(ranged.some((row)=>row.id==='old-0'),false);
  });

  it('loads that future stay through loadOwnerFinancials without a list LIMIT',async()=>{
    const past=Array.from({length:250},(_,index)=>quotedStay(`old-${index}`,'2025-02-01','2025-02-04'));
    const future=quotedStay('future-2028','2028-12-20','2028-12-27','confirmed',2540);
    const {sql,calls}=makeFinancialsSql({reservations:[...past,future],bookingFinancialsExists:false});
    const {bookings,summary}=await loadOwnerFinancials(sql,chicagoNow);
    assert.equal(bookings.some((row)=>row.id==='future-2028'),true);
    assert.equal(bookings.length,251);
    const reservationCall=calls.find((call)=>/FROM reservations/.test(call.text));
    assert.ok(reservationCall);
    assert.match(reservationCall.text,/r\.checkout > \?::date/);
    assert.doesNotMatch(reservationCall.text,/r\.checkin <= \?::date/);
    assert.equal(reservationListLimit(reservationCall.text),null);
    assert.equal(reservationCall.values[0],'2025-01-01');
    assert.equal(reservationCall.values[1],true);
    assert.equal(summary.counts.bookings,251);
  });

  it('still excludes closed stays from period and pacing totals',async()=>{
    const closed={
      ...quotedStay('cancelled-future','2026-09-20','2026-09-23','cancelled'),
      quote:{lodgingSubtotal:900,cleaningFee:240,taxes:171,total:1311,nights:3}
    };
    const active={
      ...quotedStay('active-future','2026-09-25','2026-09-28','confirmed'),
      quote:{lodgingSubtotal:1000,cleaningFee:240,taxes:186,total:1426,nights:3}
    };
    const {sql}=makeFinancialsSql({reservations:[closed,active],bookingFinancialsExists:false});
    const {bookings,summary}=await loadOwnerFinancials(sql,chicagoNow);
    assert.equal(bookings.some((row)=>row.id==='cancelled-future'),true);
    assert.equal(bookings.find((row)=>row.id==='cancelled-future').closed,true);
    assert.equal(summary.period.quotedBookings,1);
    assert.equal(summary.period.total,1426);
    const windows=pacingWindows(bookings,chicagoNow);
    assert.equal(windows[0].stays,1);
    assert.equal(windows[0].revenue,1426);
    assert.equal(windows[2].stays,1);
  });

  it('queries booking_financials by stay window without LIMIT 500',async()=>{
    const {sql,calls}=makeFinancialsSql({
      bookingFinancialsExists:true,
      imported:[{
        booking_key:'airbnb:FUTURE',
        channel:'airbnb',
        checkin:'2028-11-01',
        checkout:'2028-11-04',
        status:'confirmed',
        gross_revenue:1640,
        expected_payout:1588,
        notes:'Guest: Maria Solis'
      }]
    });
    const window=resolveFinancialsQueryWindow({now:chicagoNow});
    const result=await loadImportedFinancials(sql,window,chicagoNow);
    const importedCall=calls.find((call)=>/FROM booking_financials/.test(call.text));
    assert.ok(importedCall);
    assert.match(importedCall.text,/checkout > \?::date/);
    assert.doesNotMatch(importedCall.text,/LIMIT\s+500/);
    assert.equal(importedCall.values[0],'2025-01-01');
    assert.equal(result.available,true);
    assert.equal(result.rows.length,1);
  });

  it('removes hard list LIMITs from the Financials load path',()=>{
    const financialsSource=fs.readFileSync(path.join(__dirname,'financials.js'),'utf8');
    const apiSource=fs.readFileSync(path.join(__dirname,'../api/financials.js'),'utf8');
    const withoutQuoteSubquery=financialsSource.replace(/LEFT JOIN LATERAL \([\s\S]*?LIMIT 1\s*\) q ON true/g,'');
    assert.doesNotMatch(withoutQuoteSubquery,/LIMIT 250/);
    assert.doesNotMatch(financialsSource,/LIMIT 500/);
    assert.match(financialsSource,/listReservations/);
    assert.match(financialsSource,/resolveFinancialsQueryWindow/);
    assert.match(apiSource,/readReservationWindowParams/);
    assert.match(apiSource,/invalid_date_range/);
    assert.doesNotMatch(apiSource,/from:window\.from/);
  });
});
