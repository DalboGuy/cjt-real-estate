  ...
  schemaReady = (async () => {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS reservations (
        ...
        notes text,
        trip_type text,
        bringing_pet boolean NOT NULL DEFAULT false,
        pet_details text,
        planning_event boolean NOT NULL DEFAULT false,
        event_details text,
    ...
    await sql`ALTER TABLE reservations ADD COLUMN IF NOT EXISTS bringing_pet boolean NOT NULL DEFAULT false`;
    await sql`ALTER TABLE reservations ADD COLUMN IF NOT EXISTS pet_details text`;
    await sql`ALTER TABLE reservations ADD COLUMN IF NOT EXISTS planning_event boolean NOT NULL DEFAULT false`;
...
async function expireHolds() {
  // Booking requests remain inventory-blocking until an owner explicitly
  // accepts, rejects, cancels, or releases them. Retained as a compatibility
  // no-op for callers that previously triggered time-based expiration.
  return 0;
}

async function getActiveReservations(includeClosed = false) {
  await expireHolds();
  const sql = db();
  if (includeClosed) {
    return sql`
      SELECT id, guest_name, guest_email, guest_phone, guests, notes, trip_type, bringing_pet, pet_details, planning_event, event_details,
