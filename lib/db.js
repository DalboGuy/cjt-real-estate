...
function db() {
  // Optional app-owned override for integration-managed database variables.
  const connectionString = process.env.CJT_DATABASE_URL || process.env.DATABASE_URL;
  if (!connectionString) throw new Error('Database connection is not configured');
  if (!client) client = neon(connectionString);
  return client;
}

function isSchemaCreateRace(error) {
  return Boolean(
    error &&
    error.code === '23505' &&
    ['pg_type_typname_nsp_index', 'pg_class_relname_nsp_index'].includes(error.constraint)
  );
}

function schemaQueries(sql) {
  const queries = [
    sql`SET LOCAL lock_timeout = '3s'`,
    sql`SELECT pg_advisory_xact_lock(hashtext('cjt_owner_schema_v1'))`
  ];
    queries.push(sql`CREATE EXTENSION IF NOT EXISTS btree_gist`);
    queries.push(sql`
    ...
    `);
    queries.push(sql`ALTER TABLE reservations ADD COLUMN IF NOT EXISTS review_stage text NOT NULL DEFAULT 'pending'`);
    queries.push(sql`
      DO $$
      BEGIN
        IF EXISTS (
    ...
    `);
    queries.push(sql`ALTER TABLE owner_users ADD COLUMN IF NOT EXISTS must_change_password boolean`);
    queries.push(sql`UPDATE owner_users SET must_change_password=CASE WHEN role='admin' THEN false ELSE true END WHERE must_change_password IS NULL`);
    queries.push(sql`ALTER TABLE owner_users ALTER COLUMN must_change_password SET DEFAULT true`);
    queries.push(sql`ALTER TABLE owner_users ALTER COLUMN must_change_password SET NOT NULL`);
    ...
    `);
    queries.push(sql`ALTER TABLE owner_sessions ADD COLUMN IF NOT EXISTS user_id bigint REFERENCES owner_users(id) ON DELETE CASCADE`);
    ...
    `);
    queries.push(sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS category text`);
    queries.push(sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS recurrence text`);
    queries.push(sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS completed_at timestamptz`);
    queries.push(sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS recurrence_parent_id bigint REFERENCES tasks(id) ON DELETE SET NULL`);
    queries.push(sql`UPDATE tasks SET category='general' WHERE category IS NULL`);
    queries.push(sql`UPDATE tasks SET recurrence='none' WHERE recurrence IS NULL`);
    queries.push(sql`ALTER TABLE tasks ALTER COLUMN category SET DEFAULT 'general'`);
    queries.push(sql`ALTER TABLE tasks ALTER COLUMN category SET NOT NULL`);
    queries.push(sql`ALTER TABLE tasks ALTER COLUMN recurrence SET DEFAULT 'none'`);
    queries.push(sql`ALTER TABLE tasks ALTER COLUMN recurrence SET NOT NULL`);
    queries.push(sql`
      DO $$
      BEGIN
    ...
    queries.push(sql`
      DO $$
      BEGIN
        PERFORM pg_advisory_xact_lock(hashtext('cjt_schema_booking_payouts'));
        IF to_regclass('public.booking_payouts') IS NULL THEN
    ...
    queries.push(sql`CREATE TABLE IF NOT EXISTS communications_messages (
        id text PRIMARY KEY,
        thread_id text,
    ...
    queries.push(sql`CREATE INDEX IF NOT EXISTS communications_received_idx ON communications_messages(received_at DESC)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS communications_platform_idx ON communications_messages(platform, received_at DESC)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS communications_status_idx ON communications_messages(status, is_read, received_at DESC)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS reservations_dates_idx ON reservations(checkin, checkout)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS reservations_status_idx ON reservations(status)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS booking_events_reservation_idx ON booking_events(reservation_id, created_at DESC)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS owner_sessions_user_idx ON owner_sessions(user_id, expires_at DESC)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS tasks_status_due_idx ON tasks(status, due_at)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS tasks_assignee_status_idx ON tasks(assigned_user_id, status)`);
    queries.push(sql`CREATE TABLE IF NOT EXISTS pricing_changes (
      id text PRIMARY KEY, user_id bigint REFERENCES owner_users(id) ON DELETE SET NULL,
      actor text NOT NULL, mode text NOT NULL, note text NOT NULL DEFAULT '',
      start_date date NOT NULL, end_date date NOT NULL, entries jsonb NOT NULL,
      created_at timestamptz NOT NULL DEFAULT now()
    )`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS pricing_overrides_date_idx ON pricing_overrides(stay_date)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS booking_financials_dates_idx ON booking_financials(checkin, checkout)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS booking_financials_channel_status_idx ON booking_financials(channel, status)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS booking_payouts_channel_date_idx ON booking_payouts(channel,payout_date)`);
    queries.push(sql`CREATE INDEX IF NOT EXISTS booking_payouts_reference_idx ON booking_payouts(reservation_reference)`);
    queries.push(sql`
      DO $$
      BEGIN
...
async function ensureSchema() {
  ...
  schemaReady = (async () => {
    const sql = db();
    // One transaction holds the shared lock through every schema statement.
    // ReadCommitted lets waiting cold starts see objects created by the winner.
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        await sql.transaction(schemaQueries(sql), { isolationMode: 'ReadCommitted' });
        return;
      } catch (error) {
        // Older deployments may still initialize without the shared lock.
        if (!isSchemaCreateRace(error) || attempt === 2) throw error;
