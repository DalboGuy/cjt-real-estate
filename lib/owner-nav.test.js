...
test('booking page and guest site stay in the shell footer', () => {
  assert.equal(FOOTER_LINKS.some((item) => /booking page/i.test(item.label)), true);
  assert.equal(FOOTER_LINKS.some((item) => /guest site/i.test(item.label)), true);
});
...
test('KPI destinations carry shareable query params', () => {
  ...
  assert.equal(KPI_DESTINATIONS.financialsStays().includes('section=stays'), true);
