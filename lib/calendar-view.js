...
function dayBounds(isoDate) {
  return { start: isoDate, end: addDays(isoDate, 1) };
}
...
function viewRangeBounds(view, { year, month, focusDate }) {
  const v = normalizeView(view);
  if (v === 'week') return weekBounds(focusDate);
  if (v === 'year') return yearBounds(year);
  if (v === 'day') return dayBounds(focusDate);
...
const SYNC_MODES = Object.freeze(['full_refresh', 'view', 'probe']);

function normalizeSyncMode(mode) {
  const v = String(mode || 'view');
  return SYNC_MODES.includes(v) ? v : 'view';
}

function otaFromCaughtError(error) {
  const sources = Array.isArray(error && error.sources) && error.sources.length
    ? error.sources
    : [{
      name: 'ota',
      ok: false,
      error: error && error.message,
      missingEnv: error && error.missingEnv,
      code: error && error.code
    }];
  return { dates: new Set(), sources, events: [] };
}

function enrichSyncSource(source, checkedAt) {
  const rec = {
    name: source.name,
    label: source.label != null ? source.label : (source.name || null),
    channel: source.channel != null ? source.channel : null,
    duplicateOf: source.duplicateOf || null,
    hostHint: source.hostHint != null ? source.hostHint : null,
    origin: source.origin != null ? source.origin : null,
    ok: source.ok !== false,
    error: source.error || null,
    count: Number.isFinite(Number(source.count)) ? Number(source.count) : 0
  };
  if (source.kind) rec.kind = source.kind;
  if (source.skipped) rec.skipped = source.skipped;
  if (source.code) rec.code = source.code;
  if (source.missingEnv) rec.missingEnv = source.missingEnv;
  if (rec.ok) rec.lastSuccessfulAt = checkedAt;
  return rec;
}

function buildSyncSources(inputs, checkedAt) {
  const sources = [];
  if (Array.isArray(inputs.reservations)) {
    const liveCount = inputs.reservations.filter((r) => reservationStatusBucket(r.status) !== 'cancelled').length;
    sources.push({
      name: 'direct',
      label: 'Direct',
      channel: 'direct',
  ...
  for (const source of inputs.ota && inputs.ota.sources ? inputs.ota.sources : []) {
    sources.push(enrichSyncSource(source, checkedAt));
  }
  return sources;
}

function syncOkFromInputs(inputs) {
  if (inputs.otaConfigError) return false;
  const inbound = (inputs.ota && inputs.ota.sources ? inputs.ota.sources : []).filter(isIcalFeedSource);
  return inbound.every((source) => source.ok !== false);
}

function buildOwnerSyncPayload(inputs, options = {}) {
  const checkedAt = options.checkedAt || new Date().toISOString();
  return {
    checkedAt,
    mode: normalizeSyncMode(options.mode),
    ok: syncOkFromInputs(inputs),
...
function snapshotFromInputs(inputs, options = {}) {
  ...
  const occupancy = {
    ...
    viewedWeek: occupancyForRange(events, week.start, week.end),
    viewedYear: occupancyForRange(events, yearRange.start, yearRange.end),
    viewedDay: occupancyForRange(events, day.start, day.end),
  ...
  const window = viewRangeBounds(view, { year: y, month: m, focusDate: focus });
  ...
  return {
    ...
    occupancy,
    sync: buildOwnerSyncPayload(inputs, {
