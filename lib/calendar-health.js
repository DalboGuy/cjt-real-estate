...
function calendarHealthCopy(health = {}, { fetchFailed = false } = {}) {
  if (fetchFailed) {
    return 'Live calendar feeds could not be refreshed. Open nights stay selectable; only past dates and known blocked nights are disabled. A hold request still rechecks availability.';
  }
  const degraded = Array.isArray(health.degraded) ? health.degraded : [];
  if (degraded.length) {
    return `Calendar feeds are degraded (${degraded.join(', ')}). Open nights stay selectable; blocked nights stay blocked. A hold request still rechecks live availability.`;
  }
