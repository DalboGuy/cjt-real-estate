...
function classifyCalendarChannel({ name = '', label = '', url = '' } = {}) {
  ...
  if (hay.includes('airbnb') || compact.includes('airbnb')) {
    return { channel: 'airbnb', label: label || 'Airbnb', hostHint: calendarHostHint(url) };
  }
  if (hay.includes('vrbo') || hay.includes('homeaway') || compact.includes('vrbo') || compact.includes('homeaway')) {
...
function calendarHealthCopy(health = {}, { fetchFailed = false } = {}) {
  if (fetchFailed) {
    return 'Live calendar feeds could not be refreshed. Open nights stay selectable; only past dates and known blocked nights are disabled. A booking request still rechecks availability.';
  }
  const degraded = Array.isArray(health.degraded) ? health.degraded : [];
  if (degraded.length) {
    return `Calendar feeds are degraded (${degraded.join(', ')}). Open nights stay selectable; blocked nights stay blocked. A booking request still rechecks live availability.`;
  }
