...
async function findPotentialGuestMatches({email,phone}){
  await ensureGuestSchema();
  const sql=db();
  ...
  return sql`
    ...
    FROM guests
    WHERE phone_key=${pkey} AND (${ekey}='' OR email_key<>${ekey})
...
async function upsertGuest({name,email,phone,notes}){
  ...
  const existing=await findGuest({email:guestEmail});
  if(existing){
...
module.exports={ensureGuestSchema,findGuest,findPotentialGuestMatches,upsertGuest,linkReservationToGuest,getGuestByReservation,emailKey,phoneKey};
