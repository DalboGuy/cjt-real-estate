...
Guest booking on `/` (`booking-v2.html`) is a **direct reservation request**, not a lead form. Stripe stays deferred until after owner approval (`/complete-booking.html`).

Guest path: **Dates → Guests → Price → Reserve → Your info → Review → Reservation received**. Internally `/api/inquiries` still creates a temporary calendar hold so requested dates stay off-market while owners review. **Do not brand that as a 24-hour hold or inquiry to guests.** Primary CTA is **Reserve**. Supporting copy: you won’t be charged yet; dates are held while we review.

## Locked path

`Guest Reserve → owner queue → one owner decision → one Complete your booking link → agreement acceptance`

Owner approval alone does **not** confirm the reservation.

## What this slice does

1. **Check availability → Reserve** — guest CTAs start as **Check availability**. The calendar is the date picker (range highlight, nights counted, dates auto-carry). After both dates are set, the listing widget shows the authoritative quote and **Reserve**. Guests is an Adults/Children popover (max 14 overnight; backend still receives total guests). Checkout is progressive in guest language: Price → Your info → Review → Reserved. Submit is **Reserve**. Copy states the stay is not confirmed until the owners approve and that no payment is collected. The guest calendar only disables past dates and nights in `blockedDates`. The server rechecks availability, stores the quote, may create a temporary calendar block for owner review, **blocks those nights immediately** on `/api/calendar` and the OTA-facing `.ics` export (`Cache-Control: no-store`), and writes a **TAKE ACTION** inbox item. No payment is collected.
