...
`Guest Request to book → owner queue → one owner decision → one Complete your booking link → agreement acceptance`

Internally, `/api/inquiries` may still create a temporary calendar block so requested dates stay off-market while owners review. **Do not brand that as a 24-hour hold to guests.** Guest copy is request-to-book only. Owner approval alone does **not** confirm the reservation.

## What this slice does

1. **Check availability → Request to book → Send booking request** — guest CTAs start as **Check availability** (header + sticky). The calendar does not close on the same tap that opened it. After both dates are set, the modal closes only then and the guest is kept on the booking card (desktop scroll/focus; mobile sheet) with the all-in quote and **Request to book**. **Change dates** reopens the calendar with the current selection (no page refresh). Chat is hidden so it cannot cover that next step. Back from the calendar returns to the listing (selections preserved when dates exist). Guest count is an always-visible 1–14 stepper on the booking card and a 1–14 selector on the request form. One-night stays are valid. The request form is two steps: guest details (name, email, phone, guests, trip type, pets, parties/events, notes) then review. Back from review returns to guest details; back from guest details returns to the dates calendar. Continue is **Continue to review**. Submit is **Send booking request**. Copy states the request is not confirmed until the owners approve and that no payment is collected. The guest calendar only disables past dates and nights in `blockedDates`; a degraded Airbnb/VRBO/Booking.com feed shows a status message instead of locking the whole grid. Owner calendar connections satisfy those channels when the label or feed URL maps to them. The server rechecks availability, stores the quote, may create a temporary calendar block for owner review, **blocks those nights immediately** on `/api/calendar` and the OTA-facing `.ics` export (`Cache-Control: no-store`, no CDN stale window), and writes a **TAKE ACTION** inbox item on the owner dashboard and Direct Bookings queue. No payment is collected.
