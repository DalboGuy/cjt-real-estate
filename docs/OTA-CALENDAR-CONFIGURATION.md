...
The guest date picker does **not** disable every day when a feed is missing or failing. Only past dates and dates in `blockedDates` are disabled. A status message appears when feeds are degraded or unavailable. A booking request still rechecks live availability on submit. Calendar responses stay `Cache-Control: no-store`.
