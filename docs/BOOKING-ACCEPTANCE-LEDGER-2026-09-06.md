...
| Preview database | Preview must use official Neon branch `preview/reorg/platform-v1` via `CJT_DATABASE_URL` + `CJT_DB_TARGET=preview` (not sibling `reorg-platform-v1`, not production). Booking submissions must not create production holds. See [#19](https://github.com/DalboGuy/cjt-real-estate/issues/19) and [PREVIEW-DATABASE-SETUP.md](./PREVIEW-DATABASE-SETUP.md). | Needs review | Joel: confirm Preview env uses `CJT_DATABASE_URL` (`ep-rapid-bird`); run only approved test data and attach the evidence. |
...
| 14-guest occupancy | UI/API support 1–14. Official Preview and Production Neon branches already `CHECK` 1–14. See [#22](https://github.com/DalboGuy/cjt-real-estate/issues/22) and [FOURTEEN-GUEST-OCCUPANCY.md](./FOURTEEN-GUEST-OCCUPANCY.md). | Partial | Joel: test 1–14 on Preview against `preview/reorg/platform-v1`. Do not expect production to still be 1–12. Record `occupancy_migration_pending` only if a leftover 1–12 CHECK appears. |
...
| Guest control supports the approved 1–14 range and a 13–14 request uses the normal hold flow on the official Preview database. | **Pending / Needs review** — [ ] | **Pending / Needs review** — [ ] | **Pending / Needs review** — [ ] | Guest counts tested: ____  Screenshot/API evidence: ____  Result: ____ |
...
| Policies are readable without clipping, hidden overflow, or an auth requirement; occupancy language matches the live 1–14 CHECK (not a stale 1–12 production claim). | **Pending / Needs review** — [ ] | **Pending / Needs review** — [ ] | **Pending / Needs review** — [ ] | Mobile screenshot: ____  Guest-count note: ____  Result: ____ |
...
| [#22 — true 14-guest support](https://github.com/DalboGuy/cjt-real-estate/issues/22) | Schema CHECK is already 1–14 on official Preview and Production; remaining work is e2e Preview acceptance, not treating Production as 1–12. |

## Exact Joel runbook

1. Open the PR preview URL in a signed-out private/incognito window. Record the URL, deployed commit, and CT timestamp.
2. Confirm Preview uses `CJT_DATABASE_URL` against `preview/reorg/platform-v1` (`ep-rapid-bird`) before submitting any test inquiry/hold. Do not use production data, sibling `reorg-platform-v1`, or real payment credentials.
...
- [ ] 14-guest Preview e2e evidenced (schema already 1–14 on Preview and Production)
