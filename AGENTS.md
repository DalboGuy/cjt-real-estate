...
- **What changed:** Guest CTAs are **Check availability** until dates + quote, then **Request to book**. Guest details collect name, email, phone, guests, trip type, pets, parties/events, and notes. Continue is **Continue to review**; submit is **Send booking request**. Do **not** brand the guest path as a 24-hour hold. After dates are selected, **Change dates** reopens the calendar with the current selection (no refresh). Back from review returns to guest details; back from guest details returns to the dates calendar with selections preserved; back from dates returns to the listing. Internally a request may still create a temporary calendar block for owner review (`no-store` public calendar + OTA `.ics`) and opens a **TAKE ACTION** inbox. One owner review screen can process, approve, adjust the quote, decline, extend the hold, or release dates. Approval issues one `Complete your booking` link. Guests accept the rental agreement with an unchecked checkbox plus typed full name. Acceptance is labeled **Agreement accepted**, not signature/identity verified. Stripe charges and auto-confirm stay deferred. Guest calendar health does not fail-close every day.
