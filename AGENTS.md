...
- **What changed:** Guest CTAs are **Check availability** until dates + quote, then **Reserve**. The listing widget is the booking card (desktop sticky + mobile sticky bar). Calendar is the date picker. Guests use an Adults/Children popover; the API still receives total guests (max 14). Checkout steps: Price → Your info → Review → Reservation received. Pets/parties remain. Submit is **Reserve**. Do **not** brand the guest path as a 24-hour hold or inquiry form. Internally a request may still create a temporary calendar block for owner review (`no-store` public calendar + OTA `.ics`) and opens a **TAKE ACTION** inbox. One owner review screen can process, approve, adjust the quote, decline, extend the hold, or release dates. Approval issues one `Complete your booking` link. Guests accept the rental agreement with an unchecked checkbox plus typed full name. Acceptance is labeled **Agreement accepted**, not signature/identity verified. Stripe charges and auto-confirm stay deferred. Guest calendar health does not fail-close every day.
