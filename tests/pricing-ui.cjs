...
(async()=>{
 ...
 const originalForm=w.document.getElementById('bookingForm'),originalCheckin=originalForm.elements.checkin;let legacySubmits=0;originalForm.addEventListener('submit',()=>legacySubmits++);
 w.eval(fs.readFileSync('customer-pricing.js','utf8'));const form=w.document.getElementById('bookingForm');assert.equal(form.elements.checkin,originalCheckin);assert.equal(form.querySelectorAll('[name=trip_type]').length,1);
 ...
 form.dispatchEvent(new w.Event('submit',{cancelable:true}));await tick();assert.equal(legacySubmits,0);assert.equal(calls[1].body.expected_quote.total,1000);assert.match(form.textContent,/temporarily held/);
 ...
 form.elements.checkin.value='2091-04-02';form.elements.checkout.value='2091-04-05';form.elements.guests.value='3';await btn.onclick();const before=calls.length;form.elements.checkin.value='2091-04-03';form.dispatchEvent(new w.Event('submit',{cancelable:true}));await tick();assert.equal(calls.length,before);assert.equal(form.querySelector('#submitBtn').disabled,true);
 dom.window.close();
