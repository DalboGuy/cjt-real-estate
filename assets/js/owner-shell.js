(function(){
  ...
  if(location.pathname.replace(/\/$/,'')==='/admin-v1/maps'){
    ...
    if(version)version.textContent='Version 1.4';
    ...
    if(roadmap&&!document.getElementById('guest-listing-experience')){
      ...
      section.innerHTML='<div class="card-head"><div><h3>Guest Listing Experience</h3><p>Listing-first booking UI modeled on familiar vacation-rental interaction patterns while keeping CJT branding.</p></div><span class="badge good">v1.4</span></div><div class="map-board"><div class="map-root"><div class="map-row"><div class="map-node public built"><strong>Original Photo Mix</strong><span>pre-redesign five-photo collage restored</span></div><div class="map-arrow">→</div><div class="map-node public progress"><strong>Bedroom Cards</strong><span>large rail · Drive room folders pending</span></div><div class="map-arrow">→</div><div class="map-node public built"><strong>Full Amenity Directory</strong><span>Booking.com structure + category icons</span></div><div class="map-arrow">→</div><div class="map-node external built"><strong>Source Review Summaries</strong><span>Airbnb 4.86/5 · Booking.com 9.4/10</span></div><div class="map-arrow">→</div><div class="map-node public built"><strong>Guest Love Rail</strong><span>scrolling comments + source score</span></div><div class="map-arrow">→</div><div class="map-node external built"><strong>Grounded Map + Aerials</strong><span>dynamic leader lines to actual aerial photos</span></div><div class="map-arrow">→</div><div class="map-node public built"><strong>Sticky Book Now</strong></div></div><div class="flow-note">Desktop keeps the booking controls at the top-right with review-summary cards directly underneath. The location section calculates leader-line geometry from the property marker to the floating aerial cards and highlights that relationship on hover/focus/click. Bedroom photos are intentionally not assigned until the owner provides room-specific Google Drive folders.</div></div></div>';
      roadmap.parentNode.insertBefore(section,roadmap);
      const toolbar=document.querySelector('.map-toolbar');
    ...
    if(nextRow){const span=nextRow.querySelector('div span');if(span)span.textContent='Connect the owner-created bedroom photo folders, then complete Stripe payment/checkout, final guest confirmation, and the 14-guest production constraint migration. Communications, Documents and items 1–3 of access expansion remain deferred.';const badge=nextRow.querySelector('.badge');if(badge){badge.textContent='Booking engine';badge.classList.add('warn')}}
  }
