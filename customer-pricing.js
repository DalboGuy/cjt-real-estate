(()=>{
 ...
 // Replace the legacy submit listener so quote review precedes creating a hold.
 ...
 const trip=document.createElement('div');trip.className='field full';trip.innerHTML='<label for="tripType">Trip type</label><select id="tripType" name="trip_type" required><option value="">Select</option><option>Family trip</option><option>Friends getaway</option><option>Wedding / event</option><option>Cruise stay</option><option>Other</option></select>';phone.closest('.field').after(trip);
 ...
 const submit=form.querySelector('#submitBtn'),msg=form.querySelector('#formMessage');let quote=null,busy=false,version=0;submit.disabled=true;submit.textContent='Review price before holding dates';
 const money=n=>Number(n).toLocaleString('en-US',{style:'currency',currency:'USD'});
 const payload=()=>Object.fromEntries(new FormData(form).entries());
 function invalidate(){version++;quote=null;submit.disabled=true;submit.textContent='Review price before holding dates';out.textContent='Check availability and price for your selected dates.';}
 ...
 quoteBtn.onclick=async()=>{
  ...
  try{const d=await request('/api/quote',b);if(v!==version)return;
   ...
   quote=d.quote;out.replaceChildren();
 ...
 form.addEventListener('submit',async e=>{
  e.preventDefault();if(!quote||busy)return;const b={...payload(),expected_quote:quote};busy=true;submit.disabled=true;quoteBtn.disabled=true;msg.className='form-message show';msg.textContent='Placing your temporary hold…';
  try{const d=await request('/api/inquiries',b);msg.textContent=d.testMode?'Test complete. No reservation or hold was created.':`Your dates are temporarily held. Reference: ${d.reservation.id}. CJT will follow up with booking details.`;form.reset();invalidate();if(typeof refreshCalendar==='function')await refreshCalendar()}
