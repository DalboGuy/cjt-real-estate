...
assert.match(listingSrc, /changeDatesReview/);
...
assert.match(listingSrc, /portalGuestPopover/);
assert.match(listingSrc, /guest-popover-open/);
assert.match(listingSrc, /2147483000/);
...
for (const html of [guestHtml, indexHtml]) {
  ...
  assert.match(html, /id="changeDatesCard"/);
  assert.match(html, /Change dates/);
  ...
  assert.match(html, /id="changeDatesReview"/);
  ...
  assert.match(html, /id="guestMinus"/);
  assert.match(html, /id="guestPlus"/);
  ...
  assert.match(html, /guest-selector/);
  assert.match(html, /class="guest-popover"/);
  ...
  const selectorsEnd = html.indexOf('id="bookNowBtn"');
  const popoverAt = html.indexOf('id="guestPopover"');
  assert.ok(selectorsEnd > -1 && popoverAt > selectorsEnd, 'guest popover must be portaled outside the booking selectors');
}
...
assert.match(listingCss, /\.guest-popover\{[^}]*position:fixed/);
assert.match(listingCss, /2147483000/);
assert.match(v2Css, /#guestPopover/);
assert.match(v2Css, /guest-popover-open/);
assert.match(v2Css, /2147483000/);
